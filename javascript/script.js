...
// fetch from obj.ingredients
function getAllIngredients(recipes) {
    const ingredients = [];
    const ingredientsGroups = recipes.map((recipe) =>
        recipe.ingredients.map((ingredient) => {
            const result = ingredient.ingredient.toLowerCase();
            if (!ingredients.includes(result)) {
...
// recipe.ingedients[ingredient].map
function getAllUstensils(recipes) {
    const ustensils = recipes.map((recipe) => recipe.ustensils).flat();

    const uniqueUstensils = ustensils.filter((ustensil, index) => {
        if (ustensils.indexOf(ustensil) === index) {
            return true;
        } else return false;
    });
 ...
 * @param {Array} recipes
 */
function getAllIngredients2(recipes) {
    const ingredientsArray = [];
    const recipesList = recipes.map((recipe) => recipe.ingredients);
    recipes.map((recipe) =>
